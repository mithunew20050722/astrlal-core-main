...
app.use(session({
  ...
  store: MongoStore.create({ mongoUrl: cfg.mongoUri, collectionName: 'unity_dashboard_sessions' }),
...
// ── Control Panel Email OTP ────────────────────────────────────
const _cpOtpStore = new Map();
...
// POST /api/cp/otp/send  — send OTP via Gmail to given email
app.post('/api/cp/otp/send', async (req, res) => {
  try {
    const { email } = req.body;
    if (!email || !email.includes('@')) return res.status(400).json({ ok: false, error: 'Valid email required' });

    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    _cpOtpStore.set('admin', { otp, expires: Date.now() + 5 * 60 * 1000 });

    await _mailer.sendMail({
      from: `"UNITY-MD" <${process.env.OTP_EMAIL}>`,
      to: email,
      subject: '🔐 UNITY-MD Control Panel - OTP Code',
      html: `
        <div style="font-family:sans-serif;max-width:400px;margin:auto;background:#0a0f1a;color:#fff;border-radius:16px;padding:32px;border:1px solid #25D36640;">
          <div style="text-align:center;margin-bottom:24px;">
            <h2 style="color:#25D366;margin:12px 0 4px;">UNITY-MD</h2>
            <p style="color:#ffffff80;margin:0;">Control Panel Login</p>
          </div>
          <div style="background:#0f1520;border-radius:12px;padding:24px;text-align:center;border:1px solid #00E5FF30;margin-bottom:20px;">
            <p style="color:#ffffff80;margin:0 0 8px;font-size:13px;">Your OTP Code</p>
            <div style="font-size:36px;font-weight:900;letter-spacing:10px;color:#00E5FF;">${otp}</div>
            <p style="color:#ffffff40;margin:8px 0 0;font-size:12px;">Valid for 5 minutes</p>
          </div>
          <p style="color:#ffffff40;font-size:12px;text-align:center;margin:0;">Do not share this code with anyone.</p>
        </div>
      `,
    });
...
// POST /api/cp/otp/verify — verify OTP and set session
app.post('/api/cp/otp/verify', (req, res) => {
  try {
    const { otp } = req.body;
    if (!otp) return res.status(400).json({ ok: false, error: 'OTP required' });

    const record = _cpOtpStore.get('admin');
    if (!record) return res.status(400).json({ ok: false, error: 'OTP not found. Request a new one.' });
    if (Date.now() > record.expires) {
      _cpOtpStore.delete('admin');
...
app.post('/api/pair', async (req, res) => {
  ...
  if (existing?.status === 'connected' || existing?.status === 'pairing') {
    // Already active — stop and clear auth so a fresh pair code can be generated
    try {
      await _sm.stopSession(userId);
    } catch (_e) {}
    // Small delay to let Baileys fully close socket
  ...
  try {
    ...
    }, true /* _forceNew — re-pair always starts fresh */);
...
app.get('/api/sessions', requireAuth, async (req, res) => {
  ...
  // — scoped to THIS bot's namespace only, so dashboards never show
  // the other bot's paired numbers (Unity-MD and SL AURA share one DB).
  try {
    ...
    const inactive = docs
      .map(d => { const p = d._id.split(':'); return p.slice(1, -1).join(':'); })
...
app.post('/api/channel-react', requireAuth, async (req, res) => {
  try {
    ...
    async function tryAllReactMethods(sock, channelRawId, msgKey, msgId, emoji) {
      ...
      try {
        await sock.newsletterReactMessage(realJid, id, emoji).catch(() => {});
    ...
    async function reactOneEmoji(sock, target, emoji) {
      ...
      try {
        await sock.newsletterReactMessage(realJid, msgId, emoji).catch(() => {});
        return { ok: true, method: 1 };
      } catch (e1) {
