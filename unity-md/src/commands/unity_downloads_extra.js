  ...
  async run({ sock, m }) {
    ...
    if (['facebook', 'fb'].includes(cmd)) {
      ...
      const fbApis = [
        ...
        // 15. SnapInsta FB variant (added 2026-07)
        async () => {
          const r = await axios.post('https://snapinsta.app/action.php',
            new URLSearchParams({ url: q }),
            { headers: { 'Content-Type': 'application/x-www-form-urlencoded', Referer: 'https://snapinsta.app/', 'User-Agent': FB_UA }, timeout: 22000 }
          );
          const html = r?.data || '';
          const hdMatch = html.match(/href="(https:\/\/video[^"]+\.mp4[^"]*)"/);
          const sdMatch = html.match(/href="(https:\/\/[^"]+\.mp4[^"]*)"/);
      ...
      for (let i = 0; i < fbApis.length; i++) {
        try {
          ...
          if (fbData?.sd || fbData?.hd) {
            usedApi = ['DarkYasiya','Siputzx','Agatz','Paxsenix','Ndevapi','EliteProTech','XTeam','Cobalt','Snapsave','Fdown','Getfvid','Akuari','FastDL','GetInDevice','SnapInsta'][i] || `API${i+1}`;
