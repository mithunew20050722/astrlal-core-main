...
module.exports = {
  ...
  async run({ sock, m }) {
    ...
    if (cmd === 'hidetag' || cmd === 'htag') {
      if (!isSenderAdmin && !m.isOwner) return m.reply(`❌ *Group admins only!*\n\n${cfg.footer}`);
      let meta;
      try { meta = await sock.groupMetadata(chat); } catch { return m.reply(`❌ Couldn't fetch group info.\n\n${cfg.footer}`); }
    ...
    if (cmd === 'reject' || cmd === 'rejectreq') {
      ...
      try {
        ...
        return m.reply(`⛔ *Rejected ${jids.length} join request(s)*\n\n${cfg.footer}`);
      } catch (e) {
    ...
    if (cmd === 'addmember') {
      ...
      try {
        ...
        if (status === '200') return sock.sendMessage(chat, { text: `✅ *Added:* @${num}\n\n${cfg.footer}`, mentions: [jid] }, { quoted: msg });
        if (status === '409') return m.reply(`ℹ️ *@${num} is already in the group.*\n\n${cfg.footer}`);
        return m.reply(`⚠️ *Couldn't add @${num}* — they may have privacy settings that block group adds, or the number is invalid.\n\n${cfg.footer}`);
      } catch (e) {
    ...
    if (cmd === 'removeall' || cmd === 'kickall') {
      ...
      if (text.toLowerCase() !== 'confirm') {
        return m.reply(`⚠️ *This removes ALL non-admin members from the group!*\n\nType *.${cmd} confirm* to proceed.\n\n${cfg.footer}`);
      }
      let meta;
      try { meta = await sock.groupMetadata(chat); } catch { return m.reply(`❌ Couldn't fetch group info.\n\n${cfg.footer}`); }
      ...
      const toRemove = meta.participants
        ...
        .map(p => p.id);
      if (toRemove.length === 0) return m.reply(`ℹ️ *No members to remove.*\n\n${cfg.footer}`);
      let removed = 0;
      ...
      return sock.sendMessage(chat, { text: `✅ *Removed ${removed} member(s) from the group.*\n\n${cfg.footer}` }, { quoted: msg });
    }

    // ── KICKME / LEAVEGROUP (self-removal) ────────────────────
    if (cmd === 'kickme' || cmd === 'leavegroup') {
      if (!isBotAdmin) return m.reply(`❌ *I need to be admin to remove you.*\n\n${cfg.footer}`);
      try {
    ...
    if (cmd === 'tagadmin' || cmd === 'tgadmin') {
      ...
      if (admins.length === 0) return m.reply(`ℹ️ *No admins found.*\n\n${cfg.footer}`);
      const body = text || '📢 *Attention admins!*';
    ...
    if (cmd === 'opentime' || cmd === 'closetime') {
      ...
      try {
        if (cmd === 'opentime') {
          ...
          return m.reply(`🔓 *Group opened!* Everyone can send messages now.\n\n${cfg.footer}`);
        }
        await sock.groupSettingUpdate(chat, 'announcement');
        return m.reply(`🔒 *Group closed!* Only admins can send messages now.\n\n${cfg.footer}`);
      } catch (e) {
    ...
    if (cmd === 'joingroup' || cmd === 'joininvite') {
      if (!cfg.isOwnerNumber(senderId) && !m.isOwner) return m.reply(`❌ *Bot owner only!*\n\n${cfg.footer}`);
      ...
      try {
        ...
        return m.reply(`✅ *Joined the group successfully!*\n\n${cfg.footer}`);
      } catch (e) {
    ...
    if (cmd === 'cloud') {
      ...
      return sock.sendMessage(chat, { text: `👻 *Poof!* Your message vanished into the cloud ☁️\n\n${cfg.footer}` });
    }
  },
};
