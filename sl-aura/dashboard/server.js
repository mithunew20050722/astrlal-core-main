...
app.use(session({
  ...
  store: MongoStore.create({ mongoUrl: cfg.mongoUri, collectionName: 'sl_aura_dashboard_sessions' }),
...
app.post('/api/cp/otp/send', async (req, res) => {
  try {
    ...
    await _mailer.sendMail({
      from: `"SL AURA" <${process.env.OTP_EMAIL}>`,
      to: email,
      subject: '🔐 SL AURA Control Panel - OTP Code',
      html: `
        <div style="font-family:sans-serif;max-width:400px;margin:auto;background:#0a0f1a;color:#fff;border-radius:16px;padding:32px;border:1px solid #a855f740;">
          <div style="text-align:center;margin-bottom:24px;">
            <h2 style="color:#a855f7;margin:12px 0 4px;">SL AURA</h2>
            <p style="color:#ffffff80;margin:0;">Control Panel Login</p>
          </div>
          <div style="background:#0f1520;border-radius:12px;padding:24px;text-align:center;border:1px solid #f59e0b30;margin-bottom:20px;">
            <p style="color:#ffffff80;margin:0 0 8px;font-size:13px;">Your OTP Code</p>
            <div style="font-size:36px;font-weight:900;letter-spacing:10px;color:#f59e0b;">${otp}</div>
