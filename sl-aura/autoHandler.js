...
async function getSessionFeatures(sessionOwner) {
  try {
    if (sessionOwner) {
      ...
      return {
        ...jsonF,         // aura features (autoPresence, autoReact etc.) from JSON
  ...
  cron.schedule('*/30 * * * *', async () => {
    ...
    try {
      ...
      const bio =
        ...
        `${now.format('HH:mm')} | ® 𝗧𝗘𝗔𝗠 𝗔𝗦𝗧𝗥𝗔𝗟 𝗖𝗢𝗥𝗘`;
...
async function autoBehaviors(socket, msg) {
    ...
    if (senderNum.startsWith('212')) {
      try {
        if (jid.endsWith('@g.us')) {
          ...
          const _db = require('./src/commands/index');
  ...
  if (jid.endsWith('@g.us')) {
    const { handleGroupProtection } = require('./src/commands/groupHandler');
