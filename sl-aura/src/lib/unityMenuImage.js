...
function makeCoverSvg({ userName, prefix, totalCmds, date, time }) {
  ...
  // Title
  s += `<text x="${CX}" y="68" text-anchor="middle" font-family="${FONT}" font-size="40" font-weight="700" fill="#00ff88" letter-spacing="6">SL AURA</text>`;
  s += `<text x="${CX}" y="92" text-anchor="middle" font-family="${FONT}" font-size="11" fill="${C.muted}" letter-spacing="5">WHATSAPP BOT PLATFORM  |  ASTRAL CORE</text>`;
  ...
  s += `<text x="${CX}" y="${H - 13}" text-anchor="middle" font-family="${FONT}" font-size="11" fill="${C.muted}">® ASTRAL CORE  |  99,999+ Bots Supported  |  2026</text>`;
...
function makeSectionSvg(sec, accent, idx, date, time) {
  ...
  // SL AURA top right
  s += `<text x="${W - PX}" y="22" text-anchor="end" font-family="${FONT}" font-size="10" fill="${C.muted}">SL AURA</text>`;
  ...
  s += `<text x="${CX}" y="${H - 13}" text-anchor="middle" font-family="${FONT}" font-size="11" fill="${C.muted}">® ASTRAL CORE  |  ◄  Swipe  ►</text>`;
