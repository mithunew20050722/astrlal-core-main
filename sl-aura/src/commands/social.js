  ...
  async run({ sock, m }) {
    ...
    if (cmd === 'wasted') {
      ...
      try {
        ...
        await sock.sendMessage(chat, { image: Buffer.from(res.data), caption: `⚰️ *AURA WASTED* — @${target.split('@')[0]} 💀\n\n${cfg.footer}`, mentions: [target] }, { quoted: msg });
        await m.react('✅');
      } catch { await m.react('❌'); return m.reply(`${tr('err_failed')}\n\n${cfg.footer}`); }
      return;
    }
    ...
    if (cmd === 'roseday') {
      ...
      return sock.sendMessage(chat, { text: `🌹 *AURA Rose Day!*\n\n@${target.split('@')[0]}, here's a rose for you! 🌹🌹🌹\n\n${cfg.footer}`, mentions: [target] }, { quoted: msg });
    }
    ...
    if (cmd === 'confess' || cmd === 'confession') {
      ...
      if (m.isGroup) {
        await sock.sendMessage(chat, { text: `🌌 *AURA Anonymous Confession #${id}*\n\n_"${text}"_\n\n${cfg.footer}` });
      }
    ...
    if (cmd === 'afk') {
      ...
      return m.reply(`🌙 *AURA AFK Mode ON*\n\nReason: ${reason}\n\n${cfg.footer}`);
    }
