...
const CMD_GROUPS = {
  'AI & Search': [
    ...
    // bot AI
    'openai', 'chatgpt', 'deepseek', 'mistral', 'aurachat', 'unity', 'bot',
  ],
  'Media Tools': [
...
module.exports = {
  ...
  // Stub — settings panel currently uses button taps, not number replies
  // Returns false so messageHandler falls through to other handlers
  handleSettingsNumberReply: async (_sock, _m) => false,
...
Tap to toggle each command:`,
    ...
    // ── Main settings panel ───────────────────────────────────
    if (cmd === 'settings') {
      // FIX (2026-07): the whole handler had no error boundary — any
      // failure (DB read, sendButtons, etc.) silently produced zero
      // response. Now it always replies with SOMETHING.
      try {
        try { await sock.sendMessage(chat, { delete: m.key }); } catch {}
        await deleteSettingsButtons(sock, chat);
        ...
        await sock.sendMessage(chat, { text: `❌ *.settings failed:* ${e.message}\n\n${cfg.footer}` }).catch(() => {});
      }
      return;
    }
    ...
    // ── Set text style ────────────────────────────────────────
    if (cmd === 'settextstyle' || cmd === 'textstyle') {
      const { TEXT_STYLES } = require('./helper');
      const styles = Object.keys(TEXT_STYLES);
      if (!text) {
        const current = (await db.getBotConfig(m.sessionOwner))?.textStyle || 'elegant';
        const styleList = styles.map(s => `  ${s === current ? '✅' : '  '} .settextstyle ${s}`).join('\n');
        return m.reply(
          `🎨 *Text Style Settings*\n\n` +
          `Current: *${current}*\n\n` +
          `Available styles:\n${styleList}\n\n` +
          `Usage: *.settextstyle [style]*\n\n${cfg.footer}`
        );
      }
      const style = text.toLowerCase();
      if (!TEXT_STYLES[style]) {
        return m.reply(`❌ Unknown style. Available: ${styles.join(', ')}\n\n${cfg.footer}`);
      }
      const botCfg = await db.getBotConfig(m.sessionOwner);
      botCfg.textStyle = style;
      await botCfg.save();
      return m.reply(
        `✅ *Text style changed to:* ${style}\n\n` +
        `All bot replies will now use this style.\n\n${cfg.footer}`
      );
    }
  },
};
