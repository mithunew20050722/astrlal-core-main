...
module.exports = {
  ...
  async run({ sock, m, db: database }) {
    ...
    // ── .creator — list every creator-only command, bot-wide ───
    // Pulls straight from the live plugins map so this never goes stale:
    // any file with access:'creator' (creator.js, filemanager.js, etc.)
    // shows up here automatically, grouped by its plugin description.
    if (cmd === 'creator') {
      ...
      for (const plugin of plugins.values()) {
        if (plugin.access !== 'creator' || seen.has(plugin)) continue;
        seen.add(plugin);
        groups.push({
          title: plugin.description || 'Creator commands',
    ...
    // ── Kill ──────────────────────────────────────────────────
    if (cmd === 'kill') {
      await m.reply(`💀 *SL AURA shutting down...*\n\n${cfg.footer}`);
      logger.warn('[CREATOR] Kill command executed');
      process.exit(0);
    }

    // ── Restart ───────────────────────────────────────────────
    // {prefix}server restart — restarts ALL PM2 apps (SELECTOR, SL-AURA,
    // UNITY-MD), unlike the single-app .restart below. Extra explicit
    // number check here on top of access:'creator' — this is destructive
    // enough that it's worth double-guarding rather than trusting one gate.
    if (cmd === 'server' && (text || '').trim().toLowerCase() === 'restart') {
      if (!m.isCreator) return; // silent — creator's exact number only
      await m.reply(
        `🔄 *Full server restart initiated...*\n\n` +
        `SELECTOR, SL-AURA, UNITY-MD — all restarting now.\n` +
        `⚡ _Back online in a few seconds!_\n\n${cfg.footer}`
      );
      logger.warn('[CREATOR] Full server restart (pm2 restart all) triggered by ' + m.sender);
      exec('pm2 restart all', (err, stdout, stderr) => {
        if (err) logger.error('[CREATOR] pm2 restart all failed: ' + (stderr || err.message));
      });
      return;
    }
