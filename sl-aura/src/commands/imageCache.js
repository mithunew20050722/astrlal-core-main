'use strict';
/**
 * SL AURA — Image Pool Cache (Local Banner Edition)
 * ──────────────────────────────────────────────────────────────────────────
 * src/media/ folder ලෙස images 3ක් serve කරනවා:
 *
 *   aura_banner_1.jpg  ← Main rotation (image 1 - collage)
 *   aura_banner_2.jpg  ← Main rotation (image 2 - day scene)
 *   aura_submenu.jpg   ← Sub menu commands විතරක් (image 3 - night scene)
 *
 * getPoolImage()    → banner_1 / banner_2 alternating (every other message)
 * getSubMenuImage() → aura_submenu.jpg (sub menu commands)
 *
...
const SUBMENU_IMAGE = path.join(MEDIA_DIR, 'aura_submenu.jpg');
...
async function initImagePool() {
  console.log('[imageCache] Loading SL AURA banner images from src/media/...');
  let found = 0;
...
/**
 * getPoolImage()
 * Main rotation: aura_banner_1.jpg ↔ aura_banner_2.jpg alternating.
 */
function getPoolImage() {
  // ⚠️ TEMPORARILY DISABLED: aura_banner_1.jpg / aura_banner_2.jpg are
  // currently just renamed copies of the old UNITY-MD artwork — the
  // photo itself still has "UNITY MD (TEAM UNITY)" printed into the
  // image, since renaming a file doesn't change its pixel content.
  // Returning null here means the bot sends text-only (no banner) until
  // real SL AURA artwork is dropped into src/media/ to replace these.
  return null;

  // ── Original rotation logic (restore once real artwork is added) ──
...
/**
 * getSubMenuImage()
 * Sub menu commands විතරයි — aura_submenu.jpg.
 * Fallback: getPoolImage() (banner rotation).
 */
function getSubMenuImage() {
  // ⚠️ TEMPORARILY DISABLED — see getPoolImage() comment above.
  // aura_submenu.jpg still shows UNITY MD artwork baked into the photo.
  return null;
}
