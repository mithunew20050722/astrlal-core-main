...
global.AURA_THUMB = 'https://qu.ax/x/3Qgql.jpg';
global.sendThumb = async (sock, jid, text, quoted = null) => {
  try {
    return await sock.sendMessage(jid,
      { image: { url: global.AURA_THUMB }, caption: text },
      quoted ? { quoted } : {}
    );
  } catch (e) {}
...
async function connectToWhatsApp() {
  ...
  try {
    ...
    // ── sendMessage patch: skip list & media types ────────────────
    // Media types (audio/video/sticker/document) must NEVER get contextInfo injected
    // — WhatsApp silently drops or rejects media with status/ad contextInfo.
    const _skipContent = new Set([
      'delete','react','poll','keep','pin','unpin','star',
      'disappearingMessagesInChat','groupInviteMessage',
      // media — must pass through clean
      'audio','video','sticker','document',
    ]);
    ...
    const _CHANNEL_THUMB = global.AURA_THUMB || 'https://qu.ax/x/3Qgql.jpg';
    ...
    sock.sendMessage = async (jid, content, opts = {}) => {
      const firstKey = Object.keys(content)[0];
      // Only inject channel ad-reply for text/image — never for media or already-contextInfo'd content
      if (
        !_skipContent.has(firstKey) &&
        !opts.quoted &&
        !content.contextInfo
      ) {
        content = { ...content, contextInfo: _channelCtx() };
      }
    ...
    sock.relayMessage = async (jid, msg, opts = {}) => {
      try {
        ...
        if (im && !im.contextInfo?.externalAdReply) im.contextInfo = ctx;
        for (const t of ['conversation','extendedTextMessage','imageMessage']) {
          const node = msg[t];
          if (node && !node.contextInfo?.externalAdReply) { node.contextInfo = ctx; break; }
        }
        // audioMessage / videoMessage / documentMessage / stickerMessage — leave untouched
      } catch {}
    ...
    sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
      ...
      if (connection === 'open') {
        ...
        console.log(chalk.magenta(`[🌟] SL AURA LIVE — ${plugins.size}+ commands\n`));
        ...
        const onlineMsg =
            ...
            `└─────────────────────\n\n` +
            `✨ _SL AURA is fully loaded and ready to serve!_\n\n` +
            `${cfg.footer}`;
        ...
        setImmediate(async () => {
          try {
            ...
            // 2) Audio — local file first, fallback to URL (use _origSendMsg — bypass patch)
...
async function main() {
  ...
  // ── Telegram bots ─────────────────────────────────────────
  // NOTE: Pair Bot moved to selector/telegram/pairBot.js — disabled here.
