...
/**
 * ╔══════════════════════════════════════════════════════╗
 * ║   🎬  SL AURA — VIDEO COMMANDS  v5 (2026)           ║
...
async function fetchHQPorner(query = '', count = RESULT_COUNT) {
  try {
    ...
    const res = await axios.get(base + path, {
      timeout: 20000,
      headers: { 'User-Agent': 'Mozilla/5.0 (compatible; SL-AURA/2.0)' },
    });
    const html   = res.data;
...
async function fetchXVideos(query = '', count = RESULT_COUNT) {
  try {
    ...
    const res = await axios.get(
      ...
      { timeout: 20000, headers: { 'User-Agent': 'Mozilla/5.0 (compatible; SL-AURA/2.0)' } }
    );
    const html   = res.data;
