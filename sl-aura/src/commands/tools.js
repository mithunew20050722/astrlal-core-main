...
async function makeWebpSticker(sock, m, square = false) {
  ...
  try {
    ...
    const json = { 'sticker-pack-id': crypto.randomBytes(32).toString('hex'), 'sticker-pack-name': cfg.botName || 'SL AURA', 'sticker-pack-publisher': cfg.ownerName || 'ASTRAL CORE', 'emojis': ['🤖'] };
...
module.exports = {
  ...
  async run({ sock, m }) {
    ...
    if (cmd === 'take') {
      ...
      const packname = text || cfg.botName || 'SL AURA';
      await m.react('⏳');
      try {
        ...
        const json3 = { 'sticker-pack-id': crypto.randomBytes(32).toString('hex'), 'sticker-pack-name': packname, 'sticker-pack-publisher': cfg.ownerName || 'ASTRAL CORE', 'emojis': ['🤖'] };
    ...
    if (cmd === 'emojimix') {
      ...
      try {
        ...
        const jsonE = { 'sticker-pack-id': crypto.randomBytes(32).toString('hex'), 'sticker-pack-name': cfg.botName || 'SL AURA', 'emojis': [e1, e2] };
