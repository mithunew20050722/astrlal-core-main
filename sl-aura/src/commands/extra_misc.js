...
async function clearAllChatsOnStartup(sock) {
  return new Promise((resolve) => {
    sock.ev.once('chats.set', async ({ chats }) => {
      ...
      console.log(`[SL AURA] Startup clear: ✅ ${cleared} | ❌ ${failed}`);
...
// ══════════════════════════════════════════════════════════════
// PATCH: AURA misc features appended to run() via monkey-patch
// ══════════════════════════════════════════════════════════════
...
const _auraExtra = {
  ...
  async run({ sock, m }) {
    ...
    if (['repo','source','github2','git','github'].includes(cmd)) {
      ...
      // ── Set aura_thumb.jpg at top of pair/repo message ──────
      const _prevPoolImg = global._cmdPoolImage;
      try {
        // ⚠️ TEMPORARILY DISABLED — aura_thumb.jpg still shows UNITY MD
        // artwork baked into the photo (renamed file, not new artwork).
...
// Register aura extra commands so messageHandler can find them
if (typeof module._auraExtraRegistered === 'undefined') {
  module._auraExtraRegistered = true;
  const _origExports = module.exports;
  // Merge commands
  _origExports.commands = [...(_origExports.commands || []), ..._auraExtra.commands];
  const _origRun = _origExports.run?.bind(_origExports);
  _origExports.run = async function (ctx) {
    if (_auraExtra.commands.includes(ctx.m?.command)) {
      return _auraExtra.run(ctx);
    }
    return _origRun?.(ctx);
  };
}
