'use strict';
/**
 * SL AURA — Multi-User Session Manager
...
async function getUserAuthState(userId) {
  ...
  // Namespace by bot identity so Unity-MD and SL AURA (sharing the same
  // MongoDB) never read/write/overwrite each other's auth state.
  const ns = (cfg.sessionId || 'default').replace(/[^a-zA-Z0-9_-]/g, '');
  const docId = (key) => `${ns}:${userId}:${key}`;
...
// _isAutoRestore: true ONLY when this call comes from restoreActiveSessions()
// on boot — i.e. nobody just asked to pair, the bot is simply reconnecting
// whatever was in the DB. Used below to make sure a boot restore can never
// itself trigger a brand-new pairing-code send to a number that never
// finished pairing. Defaults to false so every other caller (.pair,
// dashboard, telegram, jadibot — real user-initiated requests) is
// byte-for-byte unchanged.
async function startSession(userId, onUpdate, _forceNew = false, _isAutoRestore = false) {
  // Don't double-start unless force (e.g. re-pair from pair site)
  if (!_forceNew && sessions.has(userId)) {
  ...
  async function connect() {
    try {
      ...
      sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
        ...
        // Generate pair code — only when first connecting, not yet registered, and no code yet
        if (connection === 'connecting' && !sock.authState.creds.registered && !session.pairCode && !session._pairRequested) {

          // FIX (2026-07): if this connect() is happening because the bot
          // just BOOTED and restoreActiveSessions() is reconnecting whatever
          // was left in the DB — NOT because a human just asked to pair —
          // and this number never actually finished pairing, do NOT send it
          // a fresh pairing code. Otherwise anyone who once typed an
          // unrelated number into the pair page (without ever entering the
          // code on that phone) gets a brand-new, unrequested pairing code
          // sent to them again on every single restart, forever.
          // This ONLY short-circuits the auto-restore path; a real .pair /
          // dashboard / telegram request always sets _isAutoRestore=false
          // (the default) and is completely unaffected — verified below by
          // requiring session.isAutoRestore to be exactly true.
          if (session.isAutoRestore === true) {
            logger.warn(`[SESSION] ${userId} never finished pairing — not auto-resending a code on restore.`);
            session.status = STATUS.DISCONNECTED;
            session._pairRequested = false;
            if (onUpdate) onUpdate(userId, { status: STATUS.DISCONNECTED, reason: 'unregistered_no_auto_resend' });
            try { sock.ws?.close?.(); } catch (_e) {}
            return;
          }

          session._pairRequested = true; // ← prevent duplicate requests
          ...
          setTimeout(async () => {
            ...
            try {
              ...
              session._pairRequested = false; // reset so reconnect can request new code
              ...
              session._pairRequested = false; // reset on error too
        ...
        if (connection === 'close') {
          ...
          session.status = STATUS.DISCONNECTED;
          session._pairRequested = false; // reset so next connect can request pair code
          ...
          // If not yet registered (still pairing), don't hammer WA — wait 30s
          if (!sock.authState.creds.registered) {
            // FIX (2026-07): this used to retry forever with no limit —
            // each retry requests a brand new pairing code that nobody is
            // there to enter, which WhatsApp's servers apparently treat as
            // abuse after enough repeats (this is very likely why OTHER,
            // genuinely-connected sessions were also getting logged out).
            // Give up after 3 attempts and wait for a real .pair request.
            session._pairAttempts = (session._pairAttempts || 0) + 1;
            if (session._pairAttempts > 3) {
              logger.warn(`[SESSION] ${userId} never completed pairing after ${session._pairAttempts} attempts — stopping, needs a fresh .pair request`);
              session.status = STATUS.ERROR;
              if (onUpdate) onUpdate(userId, { status: STATUS.ERROR, reason: 'pairing_abandoned' });
              return;
            }
            logger.info(`[SESSION] ${userId} closed before pairing — waiting 30s before retry (attempt ${session._pairAttempts}/3)`);
          ...
          if (!session.startupDone) {
            ...
            setTimeout(async () => {
              ...
              try {
                ...
                // Old default was 'inbox' — migrate existing docs to 'public'
                if (!botCfg.mode) {
                  botCfg.mode = 'public';
                  await botCfg.save().catch(() => {});
                }
                ...
                if (botCfg.langSet) {
                  ...
                  await sendButtons(sock, botJid, {
                    text:
                      ...
                      `◤◢◤◢◤◢◤◢◤◢◤◢◤◢◤◢\n` +
                      `❪❪ SL AURA ❫❫ | ® ASTRAL CORE`,
                    footer: `❪❪ SL AURA ❫❫ | ® ASTRAL CORE`,
                    buttons: [
                      { label: '🇬🇧 English', id: '.setlang en' },
                      { label: '🇱🇰 සිංහල',  id: '.setlang si' },
                      { label: '🇱🇰 தமிழ்',   id: '.setlang ta' },
                    ],
                  });

                  // ── Dedicated lang-select pending — reliable fallback ────────
                  // global.pendingButtonReplies can get overwritten by other menus.
                  // This dedicated map persists until language is actually set.
                  if (!global.langSelectPending) global.langSelectPending = new Map();
                  const _normBotJid = botJid.replace(/:\d+@/, '@');
                  global.langSelectPending.set(_normBotJid, Date.now());
...
async function clearUserSession(userId) {
  ...
  const ns = (cfg.sessionId || 'default').replace(/[^a-zA-Z0-9_-]/g, '');
  await UserAuthState.deleteMany({ _id: new RegExp(`^${ns}:${userId}:`) });
  logger.info(`[SESSION] ${userId} auth cleared`);
}

// ── Main/owner session (display-only) ───────────────────────────
// BUG FIX (2026-08): start.js already called
// global.astraSessionManager.registerMainSession(...) on every connect,
...
async function restoreActiveSessions(onUpdate) {
  // Find all unique userIds that have saved creds — scoped to THIS bot's
  // namespace only, so Unity-MD never restores SL AURA's sessions (and vice versa)
  const ns = (cfg.sessionId || 'default').replace(/[^a-zA-Z0-9_-]/g, '');
  const docs = await UserAuthState.find({ key: 'creds', _id: new RegExp(`^${ns}:`) }).lean();
  let restored = 0;
  for (const doc of docs) {
    // doc._id format: "<ns>:<userId>:creds"
    const parts  = doc._id.split(':');
    const userId = parts.slice(1, -1).join(':');
    if (!userId) continue;

    // FIX (2026-07, reverted): a previous version of this fix tried to
    // pre-filter out "never completed pairing" sessions here by checking
    // doc.data?.registered before restoring — but that ended up skipping
    // GENUINELY connected sessions too (cause not fully confirmed — possibly
    // a serialization/query edge case), which logged out working bots after
    // every restart. Reverted to restoring everything again, like before.
    // The retry-cap below (operating on the live socket's own creds, not a
    // re-derived DB projection) is what actually stops the infinite
    // pairing-code loop now — it just lets it happen up to 3 times first.
    if (!sessions.has(userId)) {
      await startSession(userId, onUpdate, false, true).catch(() => {});
      restored++;
    }
  }
