...
const { init: initAuto, autoBehaviors, handleStatus, handleCall } = require('../commands/autoHandler');
const { startDashboard } = require('../../dashboard/server');
const { start: startPairBot  } = require('./pairBot');
const { start: startMgmtBot  } = require('./managementBot');
const { start: startSuperBot } = require('./superBot');
...
async function main() {
  ...
  // NOTE: Pair Bot moved to selector/telegram/pairBot.js — it now polls
  // TG_PAIR_BOT_TOKEN from ONE place only (selector) and routes pairing to
  // this bot's dashboard API (port 3002). Starting it here too caused a
  // Telegram 409 Conflict (same token polled by 2 processes at once).
